...
onMounted(async()=>{
  try {
    const singleJobData = await fetchSingleJobData(jobId)
    console.log("single job data: ", singleJobData)
    state.job = await singleJobData
...
// onMounted(async ()=>{
//   try{
//     const singleJobData = await axios.get(`http://localhost:5000/jobs/${jobId}`)
//     state.job = singleJobData.data
//     console.log("single job data: ", state.job);
//     state.isLoading = false
